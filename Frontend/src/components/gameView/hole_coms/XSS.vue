    ...
    <el-card class="challenge-card">
      <template #header>
        <div class="card-header">
          <h3>挑战描述</h3>
          <el-tag type="warning">难度：★★☆☆☆</el-tag>
        </div>
      </template>

      <div class="challenge-content">
        <div class="description-section">
          <p>跨站脚本攻击(XSS)是一种常见的Web安全漏洞，攻击者可以在网页中注入恶意脚本，当其他用户访问该页面时，恶意脚本会在用户的浏览器中执行。本挑战将帮助你理解XSS攻击的原理和防御方法。</p>
        </div>
        ...
        <div class="task-section">
          ...
          <ul>
            <li>分析评论系统的输入过滤机制</li>
            <li>构造XSS payload绕过过滤</li>
            <li>获取其他用户的Cookie信息</li>
          </ul>
        </div>
  ...
  try {
    const response = await axios.post(
      ToUrl.url + '/lab/create-compose',
      {
        services: [
          {
            serviceName: 'xss-frontend',
            image: 'xss-frontend:latest',
            ports: { '8080': '80' }
          },
          {
            serviceName: 'xss-backend',
            image: 'xss-backend:latest',
            ports: { '3000': '3000' }
          }
        ],
        duration: 30
