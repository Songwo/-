    ...
    <el-card class="challenge-card">
      ...
      <div class="challenge-content">
        <div class="description-section">
          <p>Log4Shell是一个影响Log4j库的严重远程代码执行漏洞。本挑战将帮助你理解Log4Shell漏洞的原理、利用方式以及如何防御此类攻击。通过实践，你将学习到JNDI注入和LDAP协议的相关知识。</p>
        </div>
  ...
  try {
    const response = await axios.post(
      ToUrl.url + '/lab/create-compose',
      {
        services: [
          {
            serviceName: 'log4shell-vulnerable',
            image: 'log4shell-vulnerable:latest',
            ports: { '8080': '8080' }
          },
          {
            serviceName: 'log4shell-ldap',
            image: 'log4shell-ldap:latest',
            ports: { '1389': '1389' }
          },
          {
            serviceName: 'log4shell-http',
            image: 'log4shell-http:latest',
            ports: { '8000': '8000' }
          }
        ],
        duration: 30
  ...
  try {
    const response = await axios.post(
      ToUrl.url + '/lab/flag',
      {
        ...
        imageName: 'log4shell-vulnerable',
