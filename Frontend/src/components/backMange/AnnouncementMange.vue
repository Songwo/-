<template>
        ...
        <el-table :data="changelogs" style="width: 100%">
          ...
          <el-table-column label="操作" width="200">
            <template #default="scope">
              ...
              <el-button type="danger" size="small" @click="handleDeleteChangelog(scope.row)">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
      </el-card>
  ...
  import { getAnnouncements, createAnnouncement, updateAnnouncement, deleteAnnouncement } from '@/api/announcement.ts'
  import { getChangelogs, createChangelog, updateChangelog, deleteChangelog } from '@/api/changelog.ts'
  ...
  // 删除更新日志
  const handleDeleteChangelog = async (row: any) => {
    try {
  ...
  // 获取更新日志类型对应的标签类型
  const getChangelogType = (type: string) => {
    switch (type) {
      case 'feature':
        return 'success'
      case 'fix':
