...
public class NetworkAttacksFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.fragment_network_attacks, container, false);
            ...
            public void onResponse(Call<Result<List<Post>>> call, Response<Result<List<Post>>> response) {
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    for (Post post : allPosts) {
                        if ("网络攻击案例".equals(post.getSection())) {
                            filtered.add(post);
                        }
                    }
                    postList.clear();
